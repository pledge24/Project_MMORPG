...
- 빌드 검증: `build_project` + `filesToRebuild`로 범위를 좁힌다. `rebuild=true` 금지.
  UBT/Build.bat/MSBuild를 터미널로 직접 돌리지 않는다 — 출력 절단으로 실제 에러가 유실된다.
...
- `execute_tool_script` 사용 시 무엇을 실행할지 먼저 보고한다.
...
- `Server/GameServer/` — `*.pb.{h,cc}` + `ServerPacketHandler.h`
- `Server/DummyClient/` — `*.pb.{h,cc}` + `ClientPacketHandler.h`
- `P1/Source/P1/Network/` — `*.pb.{h,cc}` 및 `.proto` 3개
- `P1/Source/P1/` — `ClientPacketHandler.h` (`Network/`가 아니라 **모듈 루트**)

생성되는 건 패킷 핸들러 **헤더뿐**이다. `.cpp`(`Server/GameServer/ServerPacketHandler.cpp`,
...
`S_*.json`은 `Server/GameServer/`로, `C_*.json`은 `P1/Content/Gamedata/`로 **MOVE**한다.
서버는 부팅 시 `Gamedata::LoadAllGamedata()`(`Server/GameServer/Gamedata.h`)로 이 파일들을 읽어
...
**서버** — `Server/Server.sln` (x64). `Server.slnLaunch.user`에 GameServer + DummyClient를 동시에
띄우는 다중 시작 프로필이 정의되어 있다. 빌드 순서가 중요하다. `ServerCore`는 정적 라이브러리이고
`GameServer`와 `DummyClient`가 이를 링크한다. 산출물은 `Server/Binary/{Debug,Release}/`.
...
`Server/Queries/UserDB_CreateUsersTable.sql`, 게임은 `Server/GameServer/GameDB_CreateAllTables.sql`
(및 `AlterTable.sql`, `GameDB_InsertAdminAccount.sql`).
