...
산출물은 아래 9개가 전부다. 와일드카드로 적지 않는 이유는, 그렇게 적으면 생성기가 만들지
않는 파일이 같은 폴더에 있어도 문서와 모순되지 않기 때문이다.
...
갱신되지 않으므로, 발견하면 쓰이는지 확인하고 쓰이지 않으면 지운다.

장비 데이터는 `Original_Item.xlsx`의 첫 시트에 있고 세 시트가 하나의 `C_Item.json`으로 합쳐진다.
장비만 담는 별도 산출물은 없다.
