...
## `Utils/Types.h`가 `Data/`를 거꾸로 부른다
> **심각도:** 중간 · **난이도:** 낮음 · **범위:** 파일 · client
> 위치: `P1/Source/P1/Utils/Types.h` 4줄
> 등록일: 2026년 9월 21일

`docs/folder-structure.md` 3.3이 정한 의존 방향은 `게임 도메인 → Items → Data → Utils`인데,
`Utils/Types.h`가 `Data/BP_Structs.h`를 include해서 방향이 거꾸로다. `BP_Structs.h`는 `Data/`
아래 헤더 일곱 개를 모으는 집합 헤더이고, 이것을 부르는 곳은 `Utils/Types.h`와
`UI/P1ItemTooltipWidget.h` 둘뿐이다.

ADR-0007의 폴더 대칭 작업 중 `Data/`가 담는 것을 살피다가 드러났다. 그 작업의 범위가 아니어서
줄을 남겨 두었다.

### 영향

**새 기능 개발 지연** — ADR-0007이 `Data/`를 `Game/` 아래로 옮기기로 정했으므로, 그대로 두면
배선인 `Utils/`가 게임 도메인을 부르게 된다. 그 이행 티켓이 이 줄을 먼저 풀어야 진행된다.
