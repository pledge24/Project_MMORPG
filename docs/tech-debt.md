...
`Tools/ConventionLint/check_conventions.py`의 `PENDING_SERVER_MEMBER_DEBT`가 이 두 타입을 검사에서
빼고 있다. **예외가 아니라 갚을 자리로 적어 둔 것이다.** 갚으면 그 목록에서도 함께 지운다.
...
으로 지역 변수 `PC`를 선언한다. 두 함수 모두 그 뒤로 `PC`를 읽지 않는다.

#51이 위젯의 플레이어 컨트롤러 취득을 `UP1UserWidget::GetP1PlayerController`로 모으다가 드러났다.
그 티켓의 범위가 아니어서 두 줄을 남겨 두었다.

### 영향

**유지보수 어려움** — 컴파일러가 두 변수를 지우므로 실행 비용은 늘지 않는다. 남는 것은 두 함수가
...
스크립트 상수로 옮겼고, 그래서 같은 목록이 세 곳(ADR-0007 · 이 문서 · `check_conventions.py`)에
산다. 앞으로 만들 의존 방향 검사도 3.3을 그대로 읽을 수 없다.
...
## 자기 검증이 몇 건을 잡아야 하는지 보지 않는다
> **심각도:** 낮음 · **난이도:** 낮음 · **범위:** 파일 · build
> 위치: `Tools/ConventionLint/check_conventions.py`의 `SELF_TEST_FIXTURES`와 `run_self_test()`
> 등록일: 2026년 9월 22일

`SELF_TEST_FIXTURES` 위의 주석은 「검사 이름마다 (위반이 박힌 픽스처, 잡혀야 할 위반 수)를
둔다」라고 적지만 자료 구조에 건수가 없다. `run_self_test()`도 `if not dirty`만 보므로 검사가
위반을 하나라도 잡으면 통과한다.

#92가 새 검사를 만들다 여기 걸렸다. 픽스처에 위반을 셋 심었는데 그중 하나가 그 검사의 면제
목록과 같은 쌍이라 둘만 잡혔고, 그 상태로 자기 검증이 초록으로 끝났다. 픽스처를 따로 돌려
건수를 세고 나서야 알았다. 그 검사는 접었지만(`docs/backlog.md` 12번) 자기 검증의 이 성질은
그대로 남는다.

### 영향

**테스트 어려움** — 픽스처가 심은 위반 중 일부만 잡혀도 통과한다. 이 자기 검증이 막으려던
「검사가 아무것도 검사하지 않으면서 통과하는 상태」의 부분 버전이 그대로 남는다.
...
**2026년 9월 22일 덧붙임 — 부르는 쪽 폴더로 나누어 다시 세었다.** 위 표의 12건을 폴더별로
나누면 아래와 같다. 합계는 같다.
...
| `Game/Entities/` | `UI/` | 2 |
| `Game/` 아래 세 폴더 | `Protocol.pb.h` (`Entities` 1 · `Equipment` 1 · `Inventory` 1) | 3 |

**생성물을 부르는 3건은 모듈 경계의 문제가 아니라 자료형 설계의 문제다.** 도메인이 프로토콜
자료형을 그대로 쓰고 있어서, `P1`을 여러 모듈로 쪼개도 게임 모듈이 프로토콜 모듈에 의존하는
형태로 그대로 남는다. 그래서 이 3건은 나머지 9건과 갚는 방법이 다르다.

**2026년 9월 22일 두 번째 덧붙임 — 위 표의 수치를 고쳤다.** 처음 적을 때 `.pb.h` 참조를
18건으로, 합계를 27건으로 적었다. #92가 검사를 만들면서 다시 세어 보니 `Game/Data/`의 일곱
파일은 `CoreMinimal.h`와 `Engine/DataTable.h`만 부르고 `Game/Combat/`도 `.pb.h`를 부르지
않는다. `Protocol::` 식별자 출현(72회)으로도 그 식별자를 쓰는 파일 수(13개)로도 18이 나오지
않아 원래 근거를 재현하지 못했다. 고친 표는 모듈 안의 `#include` 줄을 전수로 훑어 얻었다.

**이것을 CI 검사로 막는 일은 보류했다.** #92가 그 검사를 만들었다가 접었다. 지금 넣으면 9건을
전부 면제해야 통과하는데, 면제 목록을 파일 단위로 두면 새 코드가 같은 위반을 저지를 때 목록에
한 줄 더하는 것이 부채를 갚는 것보다 싸진다. 판단 근거와 다시 잡을 조건은 `docs/backlog.md`
12번에 있다.
